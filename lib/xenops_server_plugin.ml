...
module type S = sig
	...
	module VM : sig
		...
		val save: Xenops_task.t -> progress_cb -> Vm.t -> flag list -> data -> data option -> unit
		val restore: Xenops_task.t -> progress_cb -> Vm.t -> Vbd.t list -> Vif.t list -> data -> data option -> string list -> unit
	...
	module VGPU : sig
		val start: Xenops_task.t -> Vm.id -> Vgpu.t -> bool -> unit
